...
def atu2code(bra, ald):
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.py', 'w') as fout:
        ...
        for h in ald[()]:
            ...
            if Il:
                forl=[]; forlapp=forl.append
                for i,b in enumerate(Il): forlapp(F"for {b} in range({F'{Il[i-1]},' if i else ''}np) if {b} not in [{','.join(bbl)}]")
        ...
        for kbl,vl in vld.items():
            ...
            for b in kbl:
                if ntk is not ntb: write(tabntk+F'    for {b} in range({bl[ntk-1]}+1,np):\n')
                else: write(tabntk+F'    for {b} in range(nc,np):\n')
                write(tabntk+F"        if {b} in {{{','.join(bbl)}}}: continue\n")
            ...
            for v in vl:
                ...
                for h in ald[v]:
                    ...
                    if Il:
                        forl=[]; forlapp=forl.append
                        for i,b in enumerate(Il): forlapp(F"for {b} in range({F'{Il[i-1]},' if i else ''}np) if {b} not in [{','.join(bl)}]")
...
def detat(bra, hfeqd):
    ...
    for nbc in range(1, ngb+1):  # n blocks correlation
        for nbe in range(nbc+1):  # nbe blocks from bra
            nb0=nbc-nbe  # cket=qd[nb0]  # I<J<K<L
            for bchose in combinations(bra, r=nbe):  # 从激发态取 nbe
                brest = tuple(sorted(set(bra)-set(bchose)))  # set -> list is random and disordered
