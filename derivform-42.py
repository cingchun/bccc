        ...
        for nbe in range(nbc+1):  # nbe blocks from bra
            ...
            for bchose in combinations(bra, r=nbe):  # 从激发态取 nbe
                ...
                for v,hl in _hld.items():  # 将 _hld 更新(多块加和)到 hld 中
                    ...
                    u_ = sorted(_u); bsl = [b.f for b in u_[nb1:]]  # bs for IJ...
                    bs1l = [s for s in bsl if s]; _bsl = set(bsl)
                    if bs1l != sorted(bs1l): continue  # 如果 bs 不顺序化, 丢弃
                    cket_ = prodl(qd[bsl.count(s)] for s in _bsl) if bsl else 1  # new qket for ket (bs 顺序化后的重复度 I2J2)
                    ...
                    for h in hl:
                        ...
                        h[0] = s_bef*s_lat*cket_*h[0]  # 顺序化 bs, 减少 nu, nv (AB,IJ 没有大小限制)
                        # h[1].orbsort()
                    hld[u] = hld.get(u, [])+hl
