...
#            Des:   Intermediate array based on simplified block state (unfinished)
...
from bccc.pubfun import index, sign
    ...
    def __format__(self, format_spec):
        ...
        if oper in binaryoperl:
            ...
            if oper=='+++':
                ...
                if AIl:
                    argv = F"({','.jion(AIl)}, mdl, h_mo, g_mo, pair)"
                    para = F"({','.join(_AIl)}, mdl, h_mo, g_mo, pair)"
                else: argv = '(mdl, h_mo, g_mo, pair)'

                funclinel = []
                funclinel.append(F'def {func+argv}:')
                funclinel.append(F"    # {','.join(list(chain.from_iterable(self.leafl[0])))}")
                funclinel.append( '    npair = len(pair)')
...
class Coeff(Symb):
    ...
    def __format__(self, format_spec):
        ...
        if mode is 't':
            ...
            if nsymb is 4: return F'mdl[{symbl[0].l}][{symbl[1]}][{symbl[2]},{symbl[3]}]'
            elif nsymb is 2: return F'{self.f}[{symbl[0]},{symbl[1]}]' # for mrfeqd.py
            elif nsymb is 3: return F'mdl[{symbl[1].l}][{symbl[0]}][{symbl[1].f},{symbl[2].f}]'
            else:
...
class Integral(Symb):
    ...
    def __format__(self, format_spec):
        norb = len(self.l); f = self.f; upl = {i: up.l for i, up in enumerate(self.upl)}
        sl = {i: F'pair[{upl[i]},{symb.l}]' for i, symb in enumerate(self.l)}
        if f is 2: return F'g_mo[index({sl[0]},{sl[2]},{sl[1]},{sl[3]})] \n                '
        elif f is 0: return F'h_mo[{sl[0]},{sl[1]}] \n                '
        elif f is 3: return F'g_mo[index({sl[0]},{sl[3]},{sl[1]},{sl[2]})] \n                '
        elif f is 4: return F'(g_mo[index({sl[0]},{sl[2]},{sl[1]},{sl[3]})] - g_mo[index({sl[0]},{sl[3]},{sl[1]},{sl[2]})]) \n                '
...
bfd = {
        ...
        1: Functor(symbl=(bo0a, bo1b), pdl='++'),
        2: Functor(symbl=(bo0b, bo1a), pdl='++'),
        3: Functor(symbl=(bo1a, bo1b), pdl='++'),
...
class Subbfeq(object):
    ...
    def __init__(self, bf=0, bs=0):
        self.ce = Coeff(symbl=(bf, bs), mode='inv')
        self.block = bsd[bs]
...
bfeqd = {
            bfd[0]: (Subbfeq(bf=0, bs=0), Subbfeq(bf=0, bs=1), Subbfeq(bf=0, bs=2), Subbfeq(bf=0, bs=3)),
            bfd[1]: (Subbfeq(bf=1, bs=0), Subbfeq(bf=1, bs=1), Subbfeq(bf=1, bs=2), Subbfeq(bf=1, bs=3)),
            bfd[2]: (Subbfeq(bf=2, bs=0), Subbfeq(bf=2, bs=1), Subbfeq(bf=2, bs=2), Subbfeq(bf=2, bs=3)),
            bfd[3]: (Subbfeq(bf=3, bs=0), Subbfeq(bf=3, bs=1), Subbfeq(bf=3, bs=2), Subbfeq(bf=3, bs=3)),
            bfd[4]: (Subbfeq(bf=4, bs=4),),
            bfd[5]: (Subbfeq(bf=5, bs=5),),
            bfd[6]: (Subbfeq(bf=6, bs=6),),
            bfd[7]:  (Subbfeq(bf=7,  bs=7), Subbfeq(bf=7,  bs= 8)),
            bfd[8]:  (Subbfeq(bf=8,  bs=7), Subbfeq(bf=8,  bs= 8)),
            bfd[9]:  (Subbfeq(bf=9,  bs=9), Subbfeq(bf=9,  bs=10)),
            bfd[10]: (Subbfeq(bf=10, bs=9), Subbfeq(bf=10, bs=10)),
            bfd[11]: (Subbfeq(bf=11, bs=11), Subbfeq(bf=11, bs=12)),
            bfd[12]: (Subbfeq(bf=12, bs=11), Subbfeq(bf=12, bs=12)),
            bfd[13]: (Subbfeq(bf=13, bs=13), Subbfeq(bf=13, bs=14)),
            bfd[14]: (Subbfeq(bf=14, bs=13), Subbfeq(bf=14, bs=14)),
...
class Subbseq(object):
    ...
    def __init__(self, bs=0, bf=0):
        self.ce = Coeff(symbl=(bs, bf), mode='ci')
        self.functor = bfd[bf]
...
bseqd = {
            0: (Subbseq(bs=0, bf=0), Subbseq(bs=0, bf=1), Subbseq(bs=0, bf=2), Subbseq(bs=0, bf=3)),
            1: (Subbseq(bs=1, bf=0), Subbseq(bs=1, bf=1), Subbseq(bs=1, bf=2), Subbseq(bs=1, bf=3)),
            2: (Subbseq(bs=2, bf=0), Subbseq(bs=2, bf=1), Subbseq(bs=2, bf=2), Subbseq(bs=2, bf=3)),
            3: (Subbseq(bs=3, bf=0), Subbseq(bs=3, bf=1), Subbseq(bs=3, bf=2), Subbseq(bs=3, bf=3)),
            4: (Subbseq(bs=4, bf=4),),
            5: (Subbseq(bs=5, bf=5),),
            6: (Subbseq(bs=6, bf=6),),
            7: (Subbseq(bs=7, bf=7), Subbseq(bs=7, bf=8)),
            8: (Subbseq(bs=8, bf=7), Subbseq(bs=8, bf=8)),
            9:  (Subbseq(bs=9,  bf=9), Subbseq(bs=9,  bf=10)),
            10: (Subbseq(bs=10, bf=9), Subbseq(bs=10, bf=10)),
            11: (Subbseq(bs=11, bf=11), Subbseq(bs=11, bf=12)),
            12: (Subbseq(bs=12, bf=11), Subbseq(bs=12, bf=12)),
            13: (Subbseq(bs=13, bf=13), Subbseq(bs=13, bf=14)),
            14: (Subbseq(bs=14, bf=13), Subbseq(bs=14, bf=14)),
            ...
            for rfsl in rfsll[nrfs]:
                ...
                for jbs in range(nbs):
                    ...
                    for ibs in cxbll[jbs]:
                        ...
                        cxbll[jbs][ibs] = Coeff(symbl=(nrf, ibs, jbs), mode='mdl')
                mrfeqd[nrf] = m
                rfeqd[rf] = cxbll
...
def rfeq2theory():
    ...
    with open(F'doc{ossep}{fpre}.tex', 'w') as fout:
        ...
        write(F'    There are {nrf} functors constructed: \\\\ \n')
        for irf,rf in enumerate(rfeqd.keys()):
            ...
            for q in rfeqd[rf]:
                ...
                for p in rfeqd[rf][q]:
                    write(F'    ${rfeqd[rf][q][p]!s}\\ =\\ {mrfeqd[irf][p][q]!s} $ \\\\ \n')
            write('    \n')
...
def rfeq2code(lang='Python'):
    ...
    fpre = 'mrfeqd'
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        write = fout.write
        # stdout = sys.stdout; stderr = sys.stderr; sys.stdout = fout; sys.stderr = fout
...
def taylor(ntuply):
    ...
    for n in range(ntuply+nhb+1):
        tnl = []; append = tnl.append
        for nb in range(1,n+1):
            for tn in product(range(1,ntuply+1), repeat=nb):
...
def w2theory():
    ...
    with open(F'doc{ossep}{fpre}.tex', 'w') as fout:
        ...
        write(F'    $1 + C^N_1\\times3 + C^N_2\\times{len(snd[2])} + C^N_3\\times{len(snd[3])} + C^N_4\\times{len(snd[4])} $, \\\\ \n')
        ...
        for nb in range(1, ntb+1):
            write(F'    For {nb}-block excitation, there are $ C^N_{{{nb}}}\\times{len(snd[nb])} $ excited types: \\\\ \n')
            for s in snd[nb]:
                write(F'    $\\Phi_{{{s!s}}}$ \\\\ \n')
            write('    \n')
...
def mt2code(lang='Python'):
    ...
    with open(F'bccc{ossep}{fpre}.py', 'w') as fout:
        ...
        write('def mt(pair, ntuply=4):\n')
        write('    npair = len(pair)\n')
        write('    tmp = {}\n')
        write('    \n')
...
def atu2code(bra, td):
    ...
    with open(F'bccc{ossep}at{ossep}{fpre}.py', 'w') as fout:
        ...
        write('import numpy\n')
        write('from gvb.rgvb import index\n')
        write('from bccc.pubfun import sign\n')
        write('\n\n\n\n')
        ...
        if bra:
            write(F'def {fpre}(t, h, pair, E):\n') # iterate amplitude
            write('    t1 = {}\n')
        else: write(F'def {fpre}(t, h, pair):\n') # calculate Energy
        write( '    npair = len(pair)\n')
        write( '    \n')
        ...
        nbbra = 0
        for b in bl:
            if nbbra>0:  write(tab*nbbra+F'    for {b} in range({bl[nbbra-1]}+1, npair):\n')
            else: write(F'    for {b} in range(npair):\n')
            nbbra += 1
        ...
        for kb in kbd:
            ...
            nbket = 0
            for b in il:
                if nbket>0: write(tab*(nbbra+nbket)+F'    for {b} in range({il[nbket-1]}+1, npair):\n')
...
def at2code():
    ...
    with open(F'bccc{ossep}at{ossep}{fpre}.py', 'w') as fout:
        ...
        write('def calculate(braf, t0, h, pair, E0):\n')
        write("    module = F'bccc.mh.{braf}'\n")
        write('    lib = import_module(module)\n')
        write('    fun = getattr(lib, braf)\n')
        write('    t1 = fun(t0, h, pair, E0)\n')
...
def mhuv2code(bra, ced):
    ...
    fpre = F"mh_{''.join(F'{b.l}{b.f}' for b in bra)}"
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        # stdout = sys.stdout; stderr = sys.stderr; sys.stdout = fout; sys.stderr = fout
        write = fout.write
        ...
        for ket in ced:
            ...
            ketf = F"{sket}({', '.join(ibl)}, mdl, h_mo, g_mo, pair)"
        ...
        write(F'def {fpre}(mdl, h_mo, g_mo, pair):\n')
        write( '    npair = len(pair)\n')
        write( '    tmp = {}\n')
        write( '    \n')
            ...
            for bop in deepcopy(bo_l):
                for boq in deepcopy(bo_l):
                    ...
                    #     else: keyd[key],ced[key] = key,{_inte: ce}
                                ...
                                if s_blockize is -1: ce = 0.0-ce
                                _inte = deepcopy(inte); _inte.upl = tuple(key[index(b, ibl)] for b in _ibl); _inte.orbsort()
                                try: ced[key].append(ce*_inte)
                                except KeyError: keyd[key],ced[key] = key, [ce*_inte]
    ...
    for nbcorr in range(1, nhb+1):  # n blocks correlation
        for nbexcite in range(nbcorr+1): # nbexcite blocks from bra
            ...
            for bchose in combinations(bra, r=nbexcite): # 从激发态取 nbexcite
                ...
                # if len(_ws+brest)>nhb: continue # for debug
                # print('                             _ws = ', _ws)
                ...
                print(F'_ced.keys = {_ced.keys()}')
                for key in _ced: # 将 _ced 更新(多块加和)到 ced 中
                    print(F'hash({key}) =  {hash(key)}')
                    print(F'{key} in _ced ', key in _ced)
                    ket = key+brest
                    print(F'hash({key}) =  {hash(key)}')
                    print(F'{key} in _ced ', key in _ced)
                    ...
                    if _leaf&iwb1l and (_leaf&iwb0l or _ib1l!=ib1l): # 还原激发块
                        if s_bef is 1: s_bef = Expr('-?', leaf)
                        else: s_bef = 0.0-Expr('-?', leaf)
                    for i,cxinte in enumerate(_ced[key]):
                        print('doiodoio')
                        _ced[key][i] = s_bef*cxinte

                    ket = tuple(sorted(ket))
                    # print(F'{ket} in ced ', ket in ced)
                    # print(F'{key} in _ced ', key in _ced)
                    try: ced[ket] = ced[ket]+_ced[key]
                    except KeyError: ced[ket] = _ced[key]
...
def mh2code(lang='Python'):
    ...
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        ...
        write('def calculate(braf, mdl, h_mo, g_mo, pair):\n')
        write("    module = F'bccc.mh.{braf}'\n")
        write('    lib = import_module(module)\n')
        write('    fun = getattr(lib, braf)\n')
        write('    tmp = fun(mdl, h_mo, g_mo, pair)\n')
        write('    del sys.modules[module]\n')
        write('    gc.collect()\n')
        write('    return tmp\n')
        write('\n\n')

        write('def mh(mdl, h_mo, g_mo, pair, ntuply=4):\n')
        write('    h = {}\n')
        write('    \n')

        write('    # bra wavefunction list\n')
        write('    # ground state\n')
        # write(F'    from bccc.mh.mh_ import mh_\n')
        write("    brafl = ('mh_',)\n")
        write('    \n')

        write('    # 1-block excited types\n')
        brafl = tuple(F"mh_{''.join(F'{b.l}{b.f}' for b in bra)}" for bra in snd[1])
        # for braf in brafl: write(F'    from bccc.mh.{braf} import {braf}\n')
        write('    brafl += (\n             ')
        for i,braf in enumerate(brafl):
            write(F"'{braf}', ")
            if (i+1)%5 is 0: write('\n             ')
        ...
        for ntuply in range(2,ntb+1):
            write(F'    # {ntuply}-block excited types\n')
            write(F'    if ntuply>{ntuply-1}: \n')
            brafl = tuple(F"mh_{''.join(F'{b.l}{b.f}' for b in bra)}" for bra in snd[ntuply])
            # for braf in brafl: write(F'        from bccc.mh.{braf} import {braf}\n')
            write( '        brafl += (\n                 ')
            for i,braf in enumerate(brafl):
                write(F"'{braf}', ")
        ...
        write('    pool = Pool()\n')
        write('    vl = tuple(pool.apply_async(calculate, (braf, mdl, h_mo, g_mo, pair)) for braf in brafl)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
...
def iterate2code():
    ...
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        ...
        write('    # ground state\n')
        brafl = ('mh_',)
        write('    from bccc.mh.mh_ import mh_\n')
        write('    \n')
        ...
        # write('    pool = Pool()\n')
        # write('    vl = {}\n')
        # write('    for i,braf in enumerate(brafl):  vl[i] = pool.apply_async(braf, (mdl, h_mo, g_mo, pair))\n')
        # write('    pool.close()\n')
        # write('    pool.join()\n')
