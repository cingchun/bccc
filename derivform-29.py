...
#            Des:   GVB-BCCC based on IJK <- I<J<K
...
def atu2code(bra, hld):
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.py', 'w') as fout:
        ...
        for kbl,vl in vld.items():
            ...
            for b in kbl:
                tabntk = tab*ntk
                write(tabntk+F'    for {b} in range(np):\n')
...
def at2code():
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.py', 'w') as fout:
        ...
        for ntuply in range(2,neb+1):
            write(F'    # {ntuply}-block excited types\n')
            write(F'    if ntuply>{ntuply-1}: \n')
            brafl = tuple(F"ta_{''.join(F'{b.l}{b.f}' for b in bra)}" for bra in uld[ntuply])
            for braf in brafl: write(F'        from bccc.ta.{braf} import {braf}\n')
            write( '        brafl += (\n                 ')
            for i,braf in enumerate(brafl):
                write(F'{braf}, ')
        ...
        write('        Ecorr=ta_(t0, h, p); E = Eref+Ecorr\n')
        write('        \n')
