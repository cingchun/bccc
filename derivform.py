...
#            Des:   38 -> intermediate array (del !=, de-duplication)
...
def atu2code(bra, ald):
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.py', 'w') as fout:
        ...
        #     write(tabntk+ '    \n')
        for v,al in ald.items():
            write(tabntb+F'    # {v}\n')
            # n=4; n6=math.ceil(len(al)/n)
            # _all = [al[n*i:n*(i+1)] for i in range(n6)]
            # if n6 is not 1:
            #     write(tabntb+F"    tu += sum([{','.join(F'{a}' for a in _all[0])},\n")
            #     for _al in _all[1:-1]: write(tabntb+F"               {','.join(F'{a}' for a in _al)},\n")
            #     else: write(tabntb+F"               {','.join(F'{a}' for a in _all[-1])}])\n")
            # else: write(tabntb+F"    tu += sum([{','.join(F'{a}' for a in _all[0])}])\n")
            #for a in al: write(tabntb+F"    tu += {a}\n")
            for a in al:
                write(tabntb+F"    tul.append({a})  # {na}\n")
                na += 1
            write(tabntb+ '    \n')
        ...
        for gl,h in product(gll, hl):
            ...
            for ing in range(ng-1,iI-1,-1):  # i=0: prod(thl[i:}); i=ng: sum(L0)
                ...
                for s,rtl,_inte in _pglc:
                    ...
                    ka = F'{va}'
                    # print(F' ka = {ka}')
                    ra = 0 if isinstance(inte, Integral) else iin(inte.f, ral)+1
                    kv=ad.get(ka, None)
                    if kv is None:
                        # lock.acquire()
                        a = Array(symbl=abl, mode=F'a{na}')
                        # a = Array(symbl=abl, mode=F'a{na.value}')
                        kv = [a, va]; na += 1
                        ad[ka] = kv; ral[ra].append(a.f)
                        # na.value += 1
                        # lock.release()
                ...
                for pg in pgl:
                    ...
                    pgl_.append(a)
                    print(F'a = {a}')
                ald[v] = ald.get(v, [])+[sum(pgl_)]
                for ka,(a, va) in al_.items():
                    print(F'   {a} = {va}')
                print()
...
def iter2code():
    ...
    with open(F'bccc{ossep}{fpre}.py', 'w') as fout:
        ...
        write( 'from multiprocessing import Pool, Manager\n')
        write('from bccc.pub import dei\n')
        write( 'np=None; a={}\n')
        # write( 'np=None; a=Manager().dict()\n')
        write( '\n\n')

        for a,va in ad.values():
            f=a.f; n = f[1:]
            write(F'def {f}(t, r, h, g, p, nc=0):\n')
            va = F'{va}'
            for b in a.l[::-1]:
                ib=b.l; r='range(nc,np)' if ib in b1l or b.f else 'range(np)'
                va = F'[{va} for {b.l} in {r}]'
            write(F'    a[{n}] = {va}\n')
            # write(F'    return {{{n}:{va}}}\n')
        write( '\n\n')

        write( 'def al(t, r, h, g, p, nc=0):\n')
        write( '    global np\n')
        write( '    np = len(p)\n')
        write(F"    fll = [")
        nra = iis([], ral)
        if nra is None: nra = len(ral)
        for i in range(nra):
            fl = ral[i]
            tabi0,tabi1 = '    ' if i else '', '\n' if i is not nra-1 else ''
            n = 8; n15 = math.ceil(len(fl)/n)
            _fll = [fl[n*i:n*(i+1)] for i in range(n15)]
            write(tabi0+F"[{','.join(_fll[0])},\n")
            for _fl in _fll[1:-1]: write(F"      {','.join(_fl)},\n")
            else: write(F"      {','.join(_fll[-1])}],"+tabi1)
        else: write(F"]\n")
        write( '    \n')

        write( '    for fl in fll:\n')
        write( '        for f in fl: f(t, r, h, g, p, nc)\n')
        # write( '        pool = Pool()\n')
        # write( '        pl = tuple(pool.apply(f, (t, r, h, g, p, nc)) for f in fl)\n')
        # write( '        pool.close()\n')
        # write( '        pool.join()\n')
        # write( '        for p in pl: a.update(p.get())\n')
        write( "    #for i,v in a.items(): print(F'a[{i}] = {v}')\n")
        write( '\n')
    ...
    with open(F'bccc{ossep}{fpre}.py', 'w') as fout:
        ...
        write('from collections import defaultdict as ddict\n')
        write('from bccc.al import al\n')
        write('\n\n')

        write('def iter(t, r, h, g, p, nt=4, nc=0, thd=1.0e-5, ncyc=100, xE=0.015):\n')
        write( '    # fun list\n')
        write( '    # ground state\n')
        ...
        write( '    t0 = ddict(float); udd = {u:{u:1.0} for u in t}  # all actiove t\n')
        ...
        write( '    t0.update({v:s*t[u] for u,ud in udd.items() for v,s in ud.items()})\n')
        ...
        write( '        else: \n')
        # write( '            t0.update({v:s*t[u] for u,ud in udd.items() for v,s in ud.items()})\n')
        write( '            E0=E; t0.update({v:s*t[u] for u,ud in udd.items() for v,s in ud.items()})\n')
        write( '        \n')
