...
#            Des:   Intermediate array based on simplified block state
    ...
    def __format__(self, format_spec):
        ...
        if oper in binaryoperl:
            ...
            if oper=='+++':
                ...
                if AIl:
                    argv = F"({','.jion(AIl)}, r, h_mo, g_mo, pair)"
                    para = F"({','.join(_AIl)}, r, h_mo, g_mo, pair)"
                else: argv = '(r, h_mo, g_mo, pair)'
...
class Coeff(Symb):
    ...
    def __format__(self, format_spec):
        ...
        if mode is 't':
            ...
            return F"{self.f}{''.join(F'[b.l]' for b in symbl)}"
    ...
    def __eq__(self, other):
        ...
        return F"C_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}" == F"C_{other.f}_{''.join(F'{b.l}{b.f}' for b in other.l)}"
    def __ne__(self, other):
        return F"C_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}" != F"C_{other.f}_{''.join(F'{b.l}{b.f}' for b in other.l)}"
    def __hash__(self):
        '''
        ...
        return hash(F"C_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}")
class Delta(Symb):
    '''
    Delta class
    ...
    def __eq__(self, other):
        ...
        return F"D_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}" == F"D_{other.f}_{''.join(F'{b.l}{b.f}' for b in other.l)}"
    def __ne__(self, other):
        return F"D_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}" != F"D_{other.f}_{''.join(F'{b.l}{b.f}' for b in other.l)}"
    def __hash__(self):
        '''
        ...
        return hash(F"D_{self.f}_{''.join(F'{b.l}{b.f}' for b in self.l)}")
class Integral(Symb):
    '''
    Integral class
    ...
    def __str__(self):
        ...
        if f is 2: return F'\\langle{{{sl[0]}{sl[1]}}}\\vert \\hat{{v}} \\vert{{{sl[2]}{sl[3]}}}\\rangle' # 公式输出时会自动换行 ？
            ...
            return '\\langle{{{0}{1}}}\\vert \\hat{{v}} \\vert{{{2}{3}}}\\rangle ' \
                   '- \\langle{{{0}{1}}}\\vert \\hat{{v}} \\vert{{{3}{2}}}\\rangle'.format(*sl.values())
    ...
    def orbsort(self):
        ...
        od = {i:(upl[i].l, symbl[i].l) for i in range(self.norb)} # sort orb: I0，I1, A0, A1
...
def atu2theory(bra, td):
    ...
    for ket in td:
        ...
        with open(F'doc{ossep}{fpre}.tex', 'w') as fout:
            ...
            for ixcl in td[ket]:
                for a in ixcl[1]:
                    if a in wl: continue
                        ...
                        wl.append(a)
                        sumbl = ''.join(F'\\displaystyle\\sum_{{{e}}}' for e in a.f[:index('r', a.f)] if e in iwb0l)
            ...
            write(F"    $\\langle{{{bra}}} \\vert \\hat{{H}} \\vert ({'+'.join(''.join(F'T_{t}' for t in tl) for tl in tld[len(ket)])})\\rangle = $ \\\\ \n")
            # bl = [b.l for b in ket if b.l in iwb0l]
            # sumbl = ''.join(F'\\displaystyle\\sum_{{{b}}}' for b in bl)
            for ixcl in td[ket]:
                # write(F"    ${ixcl[0]}{sumbl}{''.join(F'{e!s}' for e in ixcl[1:])} $ \\\\ \n")
                # print(F'ixcl = {ixcl}')
                if ixcl[1]:
                    a = ixcl[1][-1]
                    sa = F"X{index(a, list(iad.keys()))}({','.join(F'{b.l}' for b in a.l)})" if a.l else F'X{index(a, list(iad.keys()))}'
                else: sa = ''
                write(F"    $+{ixcl[0][0]}*{''.join(F'{e!s}' for e in ixcl[0][1:])}*{sa} $ \\\\ \n")
            write( '    \n')
...
def mhuv2code(bra, ced):
    ...
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        ...
        for ket in ced:
            ...
            ketf = F"{sket}({', '.join(ibl)}, r, h_mo, g_mo, pair)"
        ...
        write(F'def {fpre}(r, h_mo, g_mo, pair):\n')
        write( '    npair = len(pair)\n')
        write( '    tmp = {}\n')
        write( '    \n')
        ...
        for prodb in product(ws, repeat=2):
            ...
            for bop in deepcopy(bo_l):
                for boq in deepcopy(bo_l):
                    ...
                    clld={}; _keyd={} # ce {u: ce} 没有与 inte 相乘
                    for a in spinl:
                        bop.f = boq.f = a
                        bfeql = [None]*nb
                        for i,f in enumerate(functorl):
                            ...
                            for prodcxb in product(*bfeql): # 各个 block 的展开，得到系数、波函数态
                                ...
                                if key in ixcld: key = keyd[key] # 取 key of ixcld, 如果key已经有了
                                elif key in clld: key = _keyd[key] # 取 key of clld, 如果key已经有了
                                _cl = []; append = _cl.append
                                for b,c in zip(key, cl):
                                    _c = deepcopy(c)
                                    _c.l = (b,)+_c.l
                                    append(_c)
                                try: clld[key].append(_cl)  # spin 内合并 或 alpha与beta 的合并
                                except KeyError: _keyd[key],clld[key] = key, [_cl]

                    for key in clld:  # 将 clld 更新到 ixcld 中
                        _inte = deepcopy(inte)
                        _inte.upl = tuple(key[index(b, ibl)] for b in _ibl)
                        _inte.orbsort()
                        si = [s_blockize, _inte]
                        try:
                            for cl in clld[key]: ixcld[key].append(si+cl)
                        except KeyError:
                            cll = clld[key]
                            for i,cl in enumerate(cll): cll[i] = si+cl
                            keyd[key],ixcld[key] = key,cll
    ...
    for nbcorr in range(1, nhb+1):  # n blocks correlation
        for nbexcite in range(nbcorr+1): # nbexcite blocks from bra
            for bchose in combinations(bra, r=nbexcite): # 从激发态取 nbexcite
                ...
                if key in hfeqd:
                    _ixcld = deepcopy(hfeqd[key])
                    try:
                        bl = tuple(b.l for b in list(_ixcld.keys())[0])
                        if bl != iws:
                            for bl in _ixcld:
                                for b, i in zip(bl, iws): b.l = i
                    except IndexError: continue   # _ixcld = {} when nbcorr=nbexcite, b.f=6
                else:
                    _ixcld = h2wave(_ws)  # Aa I0 J0 Kk
                    hfeqd[key] = _ixcld

                for key, _ixcll in _ixcld.items(): # 将 _ixcld 更新(多块加和)到 ixcld 中
                    ket = key+brest
                    if s_bef is -1:
                        for i,_ixcl in enumerate(_ixcll): _ixcll[i][0] = -_ixcl[0]

                    ket = tuple(sorted(ket))
                    try: ixcld[ket] += _ixcll
                    except KeyError: ixcld[ket] = _ixcll

    # at
    td = {}  # {ket: t}
    ketl = set(); append = ketl.add
    for _ket in ixcld.keys(): # Aa..Jj.. -> Aa..Ij..
        ...
        for gl in tket_:
            if ket in ixcld:
                for ixcl in ixcld[ket]:
                    ...
                    _bll = [[b for b in bl if b.l in iwb0l] for bl in bll]
...
def mh2code(lang='Python'):
    ...
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        ...
        write('def calculate(braf, r, h_mo, g_mo, pair):\n')
        write("    module = F'bccc.mh.{braf}'\n")
        write('    lib = import_module(module)\n')
        write('    fun = getattr(lib, braf)\n')
        write('    tmp = fun(r, h_mo, g_mo, pair)\n')
        write('    del sys.modules[module]\n')
        write('    gc.collect()\n')
        write('    return tmp\n')
        write('\n\n')

        write('def mh(r, h_mo, g_mo, pair, ntuply=4):\n')
        write('    h = {}\n')
        write('    \n')
        ...
        write('    pool = Pool()\n')
        write('    vl = tuple(pool.apply_async(calculate, (braf, r, h_mo, g_mo, pair)) for braf in brafl)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
...
def iterate2code():
    ...
    with open(F'bccc{ossep}mh{ossep}{fpre}.py', 'w') as fout:
        ...
        # write('    pool = Pool()\n')
        # write('    vl = {}\n')
        # write('    for i,braf in enumerate(brafl):  vl[i] = pool.apply_async(braf, (r, h_mo, g_mo, pair))\n')
        # write('    pool.close()\n')
        # write('    pool.join()\n')
