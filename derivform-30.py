...
#            Des:   GVB-BCCC add core orbitals
...
gld = {nl: (reduce(mul,[qd[ni]*qd[i]**ni for i,ni in zip(sorted(set(nl)), [nl.count(i) for i in sorted(set(nl))])]),
...
def atu2code(bra, hld):
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        for v,hl in hld.items():
            ...
            nkt=0
            for b in kbl:
                write(tab*nkt+F'    for {b} in range(nc, np):\n')
        ...
        nbt = 0
        for b in bbl:
            write(tab*nbt+F'    for {b} in range(nc, np):\n')
    ...
    #     vld = {}
    #     for v in hld:
    #         kb = tuple(b.l for b in v if b.l in b0l)
    #         try: vld[kb].append(v)
    #         except KeyError: vld[kb] = [v]
    #
    #     if bra:
    #         write(F'def {fpre}(t, h, p, E, nc):\n') # Amplitude equation
    #         write( '    td = {}\n')
    #     else: write(F'def {fpre}(t, h, p, nc):\n')  # Eenergy equation
    #     write( '    np = len(p)\n')
    #     write( '    \n')
    #
    #     tab = '    '
    ...
    #                        reduce(mul, tuple(Coeff(symbl=tuple(bra[i] for i in g), mode='t') for g in gl)))
    #
    #     ntb = 0
    #     for b in bbl:
    ...
    #         tabntk = tab*ntk
    #         for v in vl:
    #             # write(tabntk+F"    tu += ({sum(hld[v])})*h[u][tuple(sorted([{','.join(F'({b.l},{b.f})' for b in v)}]))] \n")
    #             sl=[]; sapp=sl.append
    #             cl=[]; capp=cl.append
    #             for c,*l in hld[v]:
    #                 ll = [F"sorted([{','.join(b.l for b in t.l if 6<b.f<15)}])" for t in l]
    #                 sapp(F"sign({'+'.join(ll)})")
    #                 capp(c*reduce(mul, [t*Expr('-?', [b.l for b in t.l if 6<b.f<15]) for t in l]))
    #             write(tabntk+F"    tu += ({'+'.join(F'{c}*{s}' for c,s in zip(cl,sl))})*h[u][tuple(sorted([{','.join(F'({b.l},{b.f})' for b in v)}]))] \n")
    #         write(tabntk+ '    \n')
    #
    #     if bra: write(tabntb+F"    td[u] = (tu+h[()].get(u,0.0))/(E-h[u][u])-({sum(tl)})\n")
    #     write(tabntb+ '    \n')
    #
    #     if bra: write(F"    return td\n")
    #     else: write(F"    return tu\n")
    #     write('    \n\n')
    #
    #     # write('\n\n\n\n')
    #     # write("if __name__ == '__main__':\n")
...
def detat(bra, hfeqd):
    ...
    for v in vl:
        ...
        for nl in nld[nb]:
            ...
            for gl in gll:
                if all(tuple(bsl[i] for i in g) in sld[len(g)] for g in gl):
                    # append(c*reduce(mul, tuple(Coeff(symbl=tuple(Block(ordinal=bl[i], state=bsl[i]) for i in g), mode='t')
...
def at2code():
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        write('def calculate(braf, r, h, g, p, nc=0):\n')
        write("    module = F'bccc.hm.{braf}'\n")
        write('    lib = import_module(module)\n')
        write('    fun = getattr(lib, braf)\n')
        ...
        write('    pool = Pool()\n')
        write('    pl = tuple(pool.apply_async(calculate, (braf, r, h, g, p, nc)) for braf in brafl)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
