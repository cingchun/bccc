...
def mhuv2code(bra, hld):
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        for v,hl in hld.items():
            f = F"_{''.join(F'{b.l}{b.f}' for b in v)}({''.join(F'{b}, ' for b in bbl)}mf)"
            flapp(f)
            
            kbl,kket = [b.l for b in v if b.l in b0l],F"tuple(sorted([{','.join(F'{b}' for b in v)}]))"; skbl = [b.l for b in v if 6<b.f<15]
            
            write(F"def {f}:\n")
            write('    r,h,g,p,nt,nc = mf.r,mf.h,mf.g,mf.p,mf.nt,mf.nc\n')
            write( '    np = len(p)\n')
            write( '    hd = {}\n')
...
def mh2code(lang='Python'):
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        write('    pl = tuple(pool.apply_async(braf, (mf)) for braf in brafl)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
