...
#            Des:   I<J<K  -> J,K,L for intermediate array
...
def atu2code(bra, hld):
    ...
    #     kbd = {}
    #     for ket in td:
    #         kb = tuple(b.l for b in ket if b.l in b0l)
    ...
    #         for gl in gld[tl]:
    #             if all(tuple(bsl[i] for i in g) in tsd[len(g)] for g in gl):
    #                 s = sign([bl[i] for i in chain.from_iterable(gl) if 6<bsl[i]<15])
    ...
    #
    #     for kb in kbd:
    #         il = tuple(b for b in kb if b in b0l)
    #
    #         nbket = 0
    #         for b in il:
    ...
    fpre = F"hm_{''.join(F'{b.l}{b.f}' for b in bra)}"
    kbar = F"tuple(sorted([{''.join([F'({b.l}, {b.f}),' for b in bra])}]))"
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        # stdout = sys.stdout; stderr = sys.stderr; sys.stdout = fout; sys.stderr = fout
        write = fout.write
        ...
        for b in ibAl:
            # if nbbra>0:  write(tab*nbbra+F'    for {b} in range({ibAl[nbbra-1]}+1, npair):\n')
            # else: write(F'    for {b} in range(npair):\n')
            write(tab*nbbra+F'    for {b} in range(npair):\n')
            write(tab*nbbra+F"        if {b} in {{{','.join(ibAl[:nbbra])}}}: continue\n")
            nbbra += 1
        write(tab*nbbra+F'    u = {kbar}\n')
        write(tab*nbbra+ '    hd = {}\n')
        write(tab*nbbra+ '    \n')
        ...
        for ibl in ibld:
            ...
            nbket = nbbra
            for b in ibIl:
                # if nbket>nbbra: write(tab*nbket+F'    for {b} in range({ibIl[nbket-nbbra-1]}+1, npair):\n')
                # else: write(tab*nbket+F'    for {b} in range(npair):\n')
                # write(tab*nbket+F"        if {b} in {{{','.join(ibAl)}}}: continue\n")
                write(tab*nbket+F'    for {b} in range(npair):\n')
                write(tab*nbket+F"        if {b} in {{{','.join((ibAl+ibIl)[:nbket])}}}: continue\n")
                nbket += 1

            nb = nbket
            for ket in ibld[ibl]:
                _ib1l, _key, ketf = ibld[ibl][ket]
                key = F"tuple(sorted([{''.join('({}, {}),'.format(*lf) for lf in _key)}]))"

                write(tab*nb+F'    v = {key}\n')
                write(tab*nb+F'    hd[v] = hd.get(v, 0.0)+{ketf}\n')
                write(tab*nb+ '    \n')

        write(tab*nbbra+ '    # hdd[u] = hd\n')
        write(tab*nbbra+ '    try: \n')
        write(tab*nbbra+ '        for v,hv in hd.items(): hdd[u][v] = hdd[u].get(v, 0.0)+hv\n')
        write(tab*nbbra+ '    except KeyError: hdd[u] = hd\n')
        write(tab*nbbra+ '    \n')
...
def detat(bra, hfeqd):
    ...
    for nbc in range(1, ngb+1):  # n blocks correlation
        for nbe in range(nbc+1): # nbe blocks from bra
            nb0=nbc-nbe; cc=cgd[len(bra)]*cgd[nb0]
            for bchose in combinations(bra, r=nbe): # 从激发态取 nbe
                ...
                if key in hfeqd:
                    ...
                    _hld = t2w(_ws)  # Aa I0 J0 Kk
                    hfeqd[key] = _hld

                for key,ce in _hld.items(): # 将 _hld 更新(多块加和)到 hld 中
                    ket = key+brest

                    s_lat = Expr('-?', [b.l for b in ket if b.ne%2])
                    ce = s_bef*s_lat*cc*ce

                    ket = tuple(sorted(ket))
                    try: hld[ket] = hld[ket]+ce
                    except KeyError: hld[ket] = ce
...
def at2code():
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        write('    pool.close()\n')
        write('    pool.join()\n')
        write('    # for v in pl: hdd.update(v.get())\n')
        write('    for p in pl:\n')
