...
def iter(t, r, h, g, p, nt=4, nc=0, thd=1.0e-7, ncyc=500, xE=0.25):
    # fun list
    # ground state
    ...
    # libta.ta.restype = POINTER(c_double)
    # libta.ta.restype = ctypeslib.ndpointer(dtype=float64, ndim=1, flags='C_CONTIGUOUS') # bug
    ...
    ndiis=5; idiis = [*list(range(1,ndiis)), 0, ndiis]
    objl,errl = [],[]
    ...
    for it in range(ncyc):
        ...
        t0 = {v:s*t[u] for u,ud in udd.items() for v,s in ud.items()}
