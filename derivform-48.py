...
#            Des:   44 -> diis
#                   T2 such as: A1B3=A3B1=A2B3=A3B2=0; A3B3=-A4B5=-A5B4; A7B13=-A9B11; A7B14=-A9B12; A13B7=-A11B7; A8B13=-A10B11 ...
...
nel = [0,1,2] # for no T1, list(range(neb+1))
...
def atu2code(bra, ald):
    ...
    sl = [(),(1, 1), (1, 2), (2, 2), (3, 3), (6, 15), (7, 13), (7, 14), (8, 13), (8, 14)]  # For only T2
    if bsl not in sl: return
    fpre=F"ta_{sbra}"
    with open(F'bccc{ossep}ta{ossep}{fpre}.cpp', 'w') as fout:
        ...
        write(F'void {fpre}(real* t, real* r, real* h, real* g, uint* p, uint nc)\n')  # Amplitude equation
        write( '{   \n')
        write( '    \n')
        
        if bbl:
            write( '    #pragma omp parallel \n')
        ...
        write(tabntb+ '    real tu = 0.0, v=0.0;\n')
        AB = F"l{''.join(bbl)}"
...
def iter2code():
    ...
    s2l = [(), (1, 1), (1, 2), (2, 2), (3, 3), (6, 15), (7, 13), (7, 14), (8, 13), (8, 14)]  # For only T2
    s2ld = {n: tuple(''.join(F'{b.l}{b.f}' for b in u) for u in ul if tuple(b.f for b in u) in s2l) for n, ul in uld.items()}
    ft2ld = {n: tuple(F'ta_{s}' for s in sl) for n, sl in s2ld.items()}
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.h', 'w') as fout:
        ...
        write( '}\n')
        write( '\n\n')
        for ft in chain.from_iterable(ft2ld.values()): write(F'void {ft}(real* t, real* r, real* h, real* g, uint* p, uint nc=0);\n')
        write( '\n\n')
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.cpp', 'w') as fout:
        ...
        # write( '     {\n')
        for ft in chain.from_iterable(ft2ld.values()):
    ...
    with open(F'bccc{ossep}{fpre}.py', 'w') as fout:
        ...
        write(F'    sl = {s2l[1:]}\n')
        write( '    tud = [tuple(sorted(zip(bl,sl))) for bl,sl in product(permutations(list(range(np)),r=2), sl)]  # independent t\n')
        write( '    hud0 = {u:hu for u,hu in hud.items() if u in tud}\n')
        write( '    \n')
        ...
        write( '        dd = {u: (h0d.get(u, 0.0)+td[u]+(hu-E0)*sum(s*prodl([t[v] for v in vl]) for s,vl in tu))/(xE+hu-E0) for u,(hu,tu) in hud0.items()}\n')
        ...
        write( '                t = {u:sum(x[i]*objl[i][u] for i in range(ndiis)) for u in hud0}\n')
        write( '        for P in range(nc,np):\n')
        write( '            for Q in range(P+1,np):\n')
        write( '                t[((P,1),(Q,3))]=t[((P,2),(Q,3))]=t[((P,3),(Q,1))]=t[((P,3),(Q,2))]=0.0\n')
        write( '                t[((P,4),(Q,5))]=t[((P,5),(Q,4))]=-t[((P,3),(Q,3))]\n')
        write( '                t[((P,9),(Q,11))]=-t[((P,7),(Q,13))]; t[((P,11),(Q,9))]=-t[((P,13),(Q,7))]\n')
        write( '                t[((P,9),(Q,12))]=-t[((P,7),(Q,14))]; t[((P,12),(Q,9))]=-t[((P,14),(Q,7))]\n')
        write( '                t[((P,10),(Q,11))]=-t[((P,8),(Q,13))]; t[((P,11),(Q,10))]=-t[((P,13),(Q,8))]\n')
        write( '                t[((P,10),(Q,12))]=-t[((P,8),(Q,14))]; t[((P,12),(Q,10))]=-t[((P,14),(Q,8))]\n')
