...
# nel,ngl = list(range(neb+1)),list(range(neb+ngb+1))
nel,ngl = [0, 2],[0,2,4,6] # only for T2
...
def mhuv2code(bra, hld):
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        for v,hl in hld.items():
            ...
            nkt=0
            for b in kbl:
                # if nkt>0: write(tab*nkt+F'    for {b} in range({kbl[nkt-1]}+1, np):\n')
        ...
        nbt = 0
        for b in bbl:
            # if nbt>0: write(tab*nbt+F'    for {b} in range({bbl[nbt-1]}+1, np):\n')
            # else: write(F'    for {b} in range(nc, np):\n')
