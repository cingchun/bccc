        ...
        for nbe in range(nbc+1): # nbe blocks from bra
            ...
            for bchose in combinations(bra, r=nbe): # 从激发态取 nbe
                ...
                for v,hl in _hld.items(): # 将 _hld 更新(多块加和)到 hld 中
                    ...
                    if not (nbu<=neb and usl in sld[nbu]): continue
...
def detmt():
    ...
    for n in range(1, neb+1):
        for bra in uld[n]:
            ...
            mt[tuple(bra)] = at
# def mtu2code(bra, hld):
#     '''
#     matrix of Hamiltonian for T amplitude in LCC -> code
#
#     :param bra:
#     :param hld:
#     :return:
...
# def mtu2theory():
#     '''
#     matrix of Hamiltonian for T amplitude in LCC -> latex
#
#     main function
#     :return:
#     '''
#
#     raise NotImplementedError('TODO: mtu2theory()')
def mt2theory():
    '''
        ...
        for nbe in range(nbc+1): # nbe blocks from bra
            ...
            for bchose in combinations(bra, r=nbe): # 从激发态取 nbe
                ...
                for v,hl in _hld.items(): # 将 _hld 更新(多块加和)到 hld 中
                    ...
                    _u = v+deepcopy(brest); u = tuple(sorted(b for b in _u if b.f))
                    # if not set(u).issubset(set(bra)): continue # mh for LCC
                    # s_lat = Expr('-?', [b.l for b in _u if b.ne%2]) # 激发块回到原来位置
                    s_lat = sign([b.l for b in _u if b.ne%2]) # 激发块回到原来位置
                    for h in hl:
                        h[0] = s_bef*s_lat*cket*h[0]
...
if __name__ == '__main__':
    ...
    # GVB-BCCC
    func = detmh # detmh or detat
    t1 = datetime.now()
    if sys.platform=='linux':
        parallel(func)  # for parallel
