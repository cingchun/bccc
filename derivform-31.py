...
class Array(Symb):
    ...
    Array class
        ...
        such as toRepresentive matrix, Intermediate array
    :param symbl: symbol set
    '''
    ...
    for n in range(1, neb+1):
        for bra in uld[n]:
            ...
            for t in range(1,n+1):
                for comb in combinations(l, t):
                    f = tuple(bra[i] for i in comb)
                    if tuple(b.f for b in f) in sld[t]:
                        ket = tuple(b for b in bra if b not in f)
                        at[f] = (sign([b.l for b in f+ket if 6<b.f<15]), ket)
...
gld = {nl: (prodl([qd[ni]*qd[i]**ni for i,ni in zip(sorted(set(nl)), [nl.count(i) for i in sorted(set(nl))])]),
            group(set(range(n)), nl)) for n in range(neb+ngb+1) for nl in nld[n]}
ad = {}
# a = Manager().dict()
...
def atu2code(bra, ald):
    ...
    with open(F'bccc{ossep}ta{ossep}{fpre}.py', 'w') as fout:
        ...
        # h0_
        write(F'def h0_{sbra}(t, r, h, g, p, nc=0):\n')
        write( '    np = len(p)\n')
        write( '    hd = {}\n')
        write( '    \n')
        ...
        ntb = 0
        for b in bbl:
            if ntb: write(tab*ntb+F'    for {b} in range({bbl[ntb-1]}+1,np):\n')
            else: write(F'    for {b} in range(nc,np):\n')
            ntb += 1
        tabntb = tab*ntb
        ...
        # hu_
        if bra: # bra != ()
            write(F'def hu_{sbra}(t, r, h, g, p, nc=0):\n')
            write('    np = len(p)\n')
            write('    hd = {}\n')
            write('    \n')
...
def at2code():
    ...
    with open(F'bccc{ossep}hm{ossep}{fpre}.py', 'w') as fout:
        ...
        def outfl(fl):
            ...
            write( '    # 1-block excited types\n')
            f0l = f0ld[1]; f0ll = [f0l[5*i:5*(i+1)] for i in range(0, math.ceil(len(f0l)/5))]
            ...
            write( '            )\n')
            write( '    \n')
            ful = fuld[1]; full = [ful[5*i:5*(i+1)] for i in range(0, math.ceil(len(ful)/5))]
            ...
            write( '            )\n')
            write( '    \n')
            ftl = ftld[1]; ftll = [ftl[5*i:5*(i+1)] for i in range(0, math.ceil(len(ftl)/5))]
            ...
            for nt in range(2,neb+1):
                write(F'    # {nt}-block excited types\n')
                write(F'    if nt>{nt-1}: \n')
                f0l = f0ld[nt]; f0ll = [f0l[5*i:5*(i+1)] for i in range(0, math.ceil(len(f0l)/5))]
                # for f0 in f0l: write(F'    from bccc.ta.{f0} import {f0}\n')
                write( '        f0l += (\n')
                # for f0l in f0ll: write(F"                {','.join(f0l)},\n")
                for f0l in f0ll: write(F'''                {''.join(F"'{f0}'," for f0 in f0l)}\n''')
                write( '                )\n')
                write( '        \n')
                ful = fuld[nt]; full = [ful[5*i:5*(i+1)] for i in range(0, math.ceil(len(ful)/5))]
                # for fu in ful: write(F'    from bccc.ta.{fu} import {fu}\n')
                write( '        ful += (\n')
                # for ful in full: write(F"                {','.join(ful)},\n")
                for ful in full: write(F'''                {''.join(F"'{fu}'," for fu in ful)}\n''')
                write( '                )\n')
                write( '        \n')
                ftl = ftld[nt]; ftll = [ftl[5*i:5*(i+1)] for i in range(0, math.ceil(len(ftl)/5))]
                # for ft in ftl: write(F'    from bccc.ta.{ft} import {ft}\n')
                write( '        ftl += (\n')
                # for ftl in full: write(F"                {','.join(ftl)},\n")
                for ftl in full: write(F'''                {''.join(F"'{ft}'," for ft in ftl)}\n''')
...
def iter2code():
    ...
    with open(F'bccc{ossep}{fpre}.py', 'w') as fout:
        ...
        write('from copy import deepcopy\n')
        write('from datetime import datetime\n')
        write('from bccc.pub import sign,prodl\n')
        write('from itertools import permutations\n')
        write('\n\n')

        write('def iter(t0, r, h, g, p, nt=4, nc=0):\n')
        write( '    # fun list\n')
        write( '    # ground state\n')
        write(F'    from bccc.ta.ta_ import ta_\n')
        write( "    ftl = (ta_,)\n")
        ...
        write('    h0d = {}\n')
        write('    pool = Pool()\n')
        write('    l = tuple(pool.apply_async(f0, (t0, r, h, g, p, nc)) for f0 in f0l)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
        ...
        write('    hud = {}\n')
        write('    pool = Pool()\n')
        write('    l = tuple(pool.apply_async(fu, (t0, r, h, g, p, nc)) for fu in ful)\n')
        write('    pool.close()\n')
        write('    pool.join()\n')
        write('    for hu in l: hud.update(hu.get())\n')
        write('    \n')

        write('    Eshift,Eref = 0.25,h0d[()]\n')
        write('    t0 = {v: sign([b for b,s in v if 6<s<15])*tu for u,tu in deepcopy(t0).items() for v in permutations(u)}\n')
        write('    Ecorr=ta_(t0, r, h, g, p, nc)[()]; E0 = Eref+Ecorr\n')
        write("    print(F'Ecorr,E0 = {Ecorr},{E0}')\n")
        write('    \n')

        write("    print('    it            dE               dt               time/s')\n")
        write('    for it in range(500):\n')
        write('        t1 = datetime.now()\n')
        write('        # ta_\n')
        write('        pool = Pool()\n')
        ...
        write('        t = {u: (td[u]+h0d.get(u, 0.0))/(E0-hu)-sum(s*prodl([t0[v] for v in vl]) for s,vl in tu) for u,(hu,tu) in hud.items()}\n')
        write('        dt = sum(abs(t[u]-t0[u]) for u in t)\n')
        write('        \n')
...
def parallel():
    ...
    pool = Pool()
    # for u in ul: pool.apply_async(detmh, (u, hfeqd))
    for u in ul: pool.apply_async(detat, (u, hfeqd))
    pool.close()
    pool.join()
